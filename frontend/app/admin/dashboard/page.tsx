...
export default function AdminDashboard() {
  ...
  const saveCommittee = async () => {
    ...
    try {
      ...
      for (const [id, file] of photoFilesRef.current.entries()) {
        ...
        if (!resPhoto.ok) throw new Error(`Failed to upload photo for member ${id}`);
      }
  ...
  if (loading) return <p className="text-white text-center pt-24">Loading...</p>;
        ...
        <div className="bg-[#0f0f0f] border border-[#ffdc36] rounded-lg p-6">
                  ...
                  {committee.map((member) => (
                    ...
                    >
                      <div className="md:col-span-2 space-y-4">
                        ...
                        <div>
                          <label htmlFor={`member-name-${member.id}`} className="block text-sm mb-1">
                            Name
                          </label>
                          <input
                        ...
                        <div>
                          <label htmlFor={`member-bio-${member.id}`} className="block text-sm mb-1">
                            Bio
                          </label>
                          <textarea
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <input
                            ...
                            onChange={(e) => handlePhotoSelect(member.id, e.target.files?.[0] ?? null)}
                            className="text-sm"
                          />
                        </div>
                      </div>
          ...
          {activeTab === 'merch' && (
            <div>
              ...
              <p className="text-gray-400">Merchandise management coming soon...</p>
            </div>
          )}
        </div>
      </div>
